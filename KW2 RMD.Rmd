...
#Based off of this plot will choose a minimual length of 1000 to filter over 
...
#Start with ntree=500 using the assumption a tree count equal to double the number of features should be enough trees for that dataset
TreeCount <- 500
phylum_classifier_k4 <- randomForest(x = dfTraining_k4[, 5:260], y = as.factor(dfTraining_k4$Phylum), ntree = TreeCount, importance = TRUE)

#Testing the assumption enough trees were used in the calculation. 
ggplot(as.data.frame(phylum_classifier_k4$err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 
#It can bee seen the error rate has stopped rapidly decreasing indicating adding more trees will not change the analysis in a meaningful way. 
...
TreeCount <- 500
phylum_classifier_top_features_k4 <- randomForest(x = dfTraining_k4[, c("GGCC","ACCA","TTAG","TTTA")], y = as.factor(dfTraining_k4$Phylum), ntree = TreeCount, importance = TRUE)

ggplot(as.data.frame(phylum_classifier_top_features_k4$err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 

#Based off the shape of the plot, going to try again increasing the number of trees used
TreeCount <- 700
phylum_classifier_top_features_k4 <- randomForest(x = dfTraining_k4[, c("GGCC","ACCA","TTAG","TTTA")], y = as.factor(dfTraining_k4$Phylum), ntree = TreeCount, importance = TRUE)
...
#Using 700 trees produces a mroe stable decrease to the error rate so will use that going forward

#Overview of Classifier (Top 4 features):
...
TreeCount <- 500
phylum_classifier_GGCC<- randomForest(x = dfTraining_k4[, "GGCC"], y = as.factor(dfTraining_k4$Phylum), ntree = TreeCount, importance = TRUE)

#Test that enough trees were made
ggplot(as.data.frame(phylum_classifier_GGCC$err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 

#Overview of Classifier(single feature: GGCC): 
phylum_classifier_GGCC
```
```{r Model}
###Create RF model based only on the least important feature (as defined by MeanDecreaseGini): CTAC
TreeCount <- 500
phylum_classifier_CTAC<- randomForest(x = dfTraining_k4[, "CTAC"], y = as.factor(dfTraining_k4$Phylum), ntree = TreeCount, importance = TRUE)

ggplot(as.data.frame(phylum_classifier_CTAC$err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 

#Overview of Classifier(single feature: CTAC): 
...
TreeCount <- 200
phylum_classifier_k3 <- randomForest(x = dfTraining_k3[, 5:68], y = as.factor(dfTraining_k3$Phylum), ntree = TreeCount, importance = TRUE)

ggplot(as.data.frame(phylum_classifier_k3 $err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 
...
TreeCount = 100
phylum_classifier_k2 <- randomForest(x = dfTraining_k2[, 5:20], y = as.factor(dfTraining_k2$Phylum), ntree = TreeCount, importance = TRUE)

ggplot(as.data.frame(phylum_classifier_k2 $err.rate), aes(x= 1:TreeCount, y = OOB)) +geom_point(size = 0.5, shape = 1) + 
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black")) + labs(x="Number of trees", y = "Error rate") 

#Could look better. Going to increase the tree count.
TreeCount = 500
phylum_classifier_k2 <- randomForest(x = dfTraining_k2[, 5:20], y = as.factor(dfTraining_k2$Phylum), ntree = TreeCount, importance = TRUE)
...
#Staisifed with these error rates
