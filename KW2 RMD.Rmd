...
### Going to require a test for ntrees, for now leaving set to 600 as an approximation 
...
importance<-phylum_classifier_k4$importance
...
df_importance_k4 <- importance(phylum_classifier_k4) %>% 
  data.frame() %>% 
...
#Again pick ntree not arbitrarily 
...
phylum_classifier_GGCC<- randomForest(x = dfTraining_k4[, "GGCC"], y = as.factor(dfTraining$Phylum), ntree = 600, importance = TRUE)
...
phylum_classifier_CTAC<- randomForest(x = dfTraining[, "CTAC"], y = as.factor(dfTraining$Phylum), ntree = 600, importance = TRUE)
...
knn_prediction1<-knn(train=dfTraining_k4[,5:260],test=dfvalidation_k4[,5:260],cl=dfTraining$Phylum,k=3)
